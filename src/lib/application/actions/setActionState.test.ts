...
describe('setActionState (application use case)', () => {
	...
	it('delegates reopening (newState: OPEN) the same way', () => {
		const resultAction = { id: 'a1', state: 'OPEN' } as Action;
		const actions: ActionRepositoryPort = {
			listActions: vi.fn(),
			listDependencies: vi.fn(),
			setActionState: vi.fn(() => resultAction),
		...
		const result = setActionState(
			{ actions },
			{ itemId: 'item-1', actionId: 'a1', newState: 'OPEN' }
		);

		expect(actions.setActionState).toHaveBeenCalledWith('item-1', 'a1', 'OPEN');
		expect(result).toBe(resultAction);
	});

	it('lets a repository transition error propagate', () => {
