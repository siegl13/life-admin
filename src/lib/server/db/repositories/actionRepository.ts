 ...
 * Only OPEN -> DONE and OPEN -> SKIPPED are allowed in V1 (no reopening)
 * — see domain/action/transitions.ts, whose own transition table this
 * reads via `statesThatCanTransitionTo` rather than re-encoding it.
 */
export function setActionState(
	...
	const result = db
		...
		.run(newState, now, actionId, ...validFromStates, itemId);
